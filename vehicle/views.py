...
def index(request):
	...
	connection = cx_Oracle.connect("SYSTEM/Amogh@2001@localhost/orcl")
	...
	if request.method == 'POST':
		...
		if 'adminId' in request.POST:
			...
			if 'firstName' not in request.POST:
				return render(request, 'vehicle/adminSignUp.html', {
					'adminId': adminId1,
				})
		...
		if adminId1:
			return HttpResponseRedirect(reverse('vehicle:adminHomePage', args=(adminId1,)))
		elif updateId:
			return HttpResponseRedirect(reverse('vehicle:adminHomePage', args=(updateId,)))

		return HttpResponseRedirect(reverse('vehicle:adminLogin'))
	...
	if request.method == 'POST':
		...
		if checkLst:
			...
			if passWord != inputLst[1]:
				...
				return HttpResponseRedirect(reverse('vehicle:adminHomePage', args=(adminId,)))
		else:
...
def adminHomePage(request, admin_id):
	...
	return render(request, 'vehicle/adminHomePage.html', {
		'userInfo': userInfo,
		'carLst': carLst,
	})
	...
	if request.method == 'POST':
		...
		if 'updateId' in request.POST:
			...
			if 'firstName' not in request.POST:
				...
				return render(request, 'vehicle/userSignUp.html', {
					'inputLst': inputLst,
					'updateId': updateId,
					'errorLst': errorLst,
					'noError': "OK",
		...
		if updateId:
			return HttpResponseRedirect(reverse('vehicle:userHomePage', args=(updateId,)))

		return HttpResponseRedirect(reverse('vehicle:userLogin'))
	...
	if request.method == 'POST':
		...
		if 'updateId' in request.POST:
			...
			if 'firstName' not in request.POST:
				...
				return render(request, 'vehicle/driverSignUp.html', {
					'inputLst': inputLst,
					'updateId': updateId,
					'errorLst': errorLst,
					'noError': "OK",
		...
		if updateId:
			return HttpResponseRedirect(reverse('vehicle:driverHomePage', args=(updateId,)))

		return HttpResponseRedirect(reverse('vehicle:driverLogin'))
	...
	if request.method == 'POST':
		...
		if checkLst:
			...
			if passWord != inputLst[1]:
				...
				return HttpResponseRedirect(reverse('vehicle:userHomePage', args=(userId,)))
		else:
			errorLst.append("Invalid username/mobile number/email")
		...
		return render(request, 'vehicle/userLogin.html', {
			'inputLst': inputLst,
			'errorLst': errorLst,
		})
	...
	if request.method == 'POST':
		...
		if checkLst:
			...
			if passWord != inputLst[1]:
				...
				return HttpResponseRedirect(reverse('vehicle:driverHomePage', args=(driverId,)))
		else:
			errorLst.append("Invalid username/mobile number/email")
		...
		return render(request, 'vehicle/driverLogin.html', {
			'inputLst': inputLst,
			'errorLst': errorLst,
		})
...
# ======================================================================= change ======================================================
	...
	return render(request, 'vehicle/userHomePage.html', {
		'userInfo': userInfo,
...
# ======================================================================= change ======================================================
		...
		return render(request, 'vehicle/driverHomePage.html', {
...
def newRequest(request, user_id):
	...
	return HttpResponseRedirect(reverse('vehicle:userHomePage', args=(checkLst[0][0],)))
...
def acceptRequest(request, request_id, driver_id):
	...
	return HttpResponseRedirect(reverse('vehicle:driverHomePage', args=(driver_id,)))
...
def userCancelRequest(request, request_id, user_id):
	...
	return HttpResponseRedirect(reverse('vehicle:userHomePage', args=(user_id,)))
	...
	if request.method == "POST":
		...
		if 'admin' in request.POST:
			...
			if 'namePlate' not in request.POST:
				return render(request, 'vehicle/rentCar.html', {'admin': admin,})
		...
		if not isOK:
			return render(request, 'vehicle/rentCar.html', {
				'inputLst': inputLst,
				'errorLst': errorLst,
				'userId' : user_id,
				'admin' : admin,
		...
		if admin:
			return HttpResponseRedirect(reverse('vehicle:adminHomePage', args=(admin,)))
		else:
			return HttpResponseRedirect(reverse('vehicle:userHomePage', args=(user_id,)))



	return render(request, 'vehicle/rentCar.html', {'userId': user_id,})
...
def userCarInfo(request, user_id):
	...
	return render(request, 'vehicle/userCarInfo.html', {
		'inputLst' : carLst,
		'userId' : user_id,
	})
...
def userDeleteCar(request, nameplate, user_id):
	...
	return HttpResponseRedirect(reverse('vehicle:userCarInfo', args=(user_id,)))
...
def carRentRequest(request, admin_id):
	...
	return render(request, 'vehicle/carRentRequest.html', {
		'carLst': carLst,
		'userId': admin_id,
	})
...
def adminApproveCar(request, nameplate, admin_id):
	...
	return HttpResponseRedirect(reverse('vehicle:carRentRequest', args=(admin_id,)))
...
def driverPickCar(request, driver_id):
	...
	if request.method == "POST":
		...
		return render(request, 'vehicle/driverPickCar.html', {
			'driverId': driver_id,
			'carLst': carLst,
		})
...
def driverRateUser(request, request_id, driver_id):
	return render(request, 'vehicle/driverRateUser.html', {
...
def userRateDriver(request, user_id, request_id, driver_id):
	if request.method == "POST":
		...
		if 'rating' in request.POST:
			...
			return render(request, 'vehicle/userRateDriver.html', {
				'requestId': request_id,
				'driverId': driver_id,
				'userId': user_id,
			})
...
def adminPayCar(request, nameplate, admin_id):
	if request.method == "POST":
		...
		if data:
			...
			return  HttpResponseRedirect(reverse('vehicle:adminHomePage', args=(admin_id,)))
...
def userRideHistory(request, user_id):
	...
	userData = cursor.fetchall()
	if userData:
		return render(request, 'vehicle/userRideHistory.html', {
			'userId': user_id,
			'rides': data,
			'More': More,
			'Ended': Ended,
...
def userDriverInfo(request, user_id, driver_id):
	...
	if data:
		...
		return render(request, 'vehicle/userDriverInfo.html', {
			'userId': user_id,
			'driverInfo': data,
		})

	return HttpResponseRedirect(reverse('vehicle:userRideHistory', args=(user_id,)))
...
def driverRideHistory(request, driver_id):
	...
	userData = cursor.fetchall()
	if userData:
		return render(request, 'vehicle/driverRideHistory.html', {
			'driverId': driver_id,
			'rides': data,
			'More': More,
			'Ended': Ended,
...
def driverUserInfo(request, driver_id, user_id):
	...
	if data:
		...
		return render(request, 'vehicle/driverUserInfo.html', {
			'driverId': driver_id,
			'driverInfo': data,
		})
	#driverInfo contains userInfo

	return HttpResponseRedirect(reverse('vehicle:driverRideHistory', args=(driver_id,)))
...
def carInfoAdmin(request, admin_id):
	...
	return render(request, 'vehicle/carInfoAdmin.html', {
		'data': data,
		'More': More,
		'userId': admin_id,
		'search': search,
...
def adminDeleteCar(request, nameplate, admin_id):
	...
	return HttpResponseRedirect(reverse('vehicle:carInfoAdmin', args=(admin_id,)))
...
def userInfoAdmin(request, admin_id):
	...
	return render(request, 'vehicle/userInfoAdmin.html', {
		'data': data,
		'More': More,
		'userId': admin_id,
		'search': search,
...
def adminDeleteUser(request, user_id, admin_id):
	...
	return HttpResponseRedirect(reverse('vehicle:userInfoAdmin', args=(admin_id,)))
...
def driverInfoAdmin(request, admin_id):
	...
	return render(request, 'vehicle/driverInfoAdmin.html', {
		'data': data,
		'More': More,
		'userId': admin_id,
		'search': search,
...
def adminDeleteDriver(request, driver_id, admin_id):
	...
	return HttpResponseRedirect(reverse('vehicle:driverInfoAdmin', args=(admin_id,)))